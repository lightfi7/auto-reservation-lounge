...
def create_task(request: Request, task: Emulator):
    task = jsonable_encoder(task)
    new_task = request.app.database["emulators"].insert_one(task)
    created_task = request.app.database["emulators"].find_one({"_id": new_task.inserted_id})
    return created_task

@router.put("/{emulator_id}", response_model=Emulator)
def update_task(request: Request, emulator_id: str, task: Emulator):
    task = {k: v for k, v in task.dict().items() if v is not None}
    if len(task) != 0:
        updated_result = request.app.database["emulators"].update_one({"_id": emulator_id}, {"$set": task})
...
def delete_task(request: Request, emulator_id: str):
    deleted_task = request.app.database["emulators"].delete_one({"_id": emulator_id})
